...
const { PaymentMobile } = NativeModules;
...
// const PaymentMobileEvents = new NativeEventEmitter(NativeModules.PaymentMobile);

// PaymentMobileEvents.addListener(
...
export default class PaymentMobileComponent extends Component {
  static propTypes = {
  ...
  //   const {mode, urlScheme} = this.props;
    // PaymentMobile.initPaymentProvider(mode);
//     PaymentMobile.setUrlScheme(urlScheme);
// }
  ...
  async submitTransaction(transaction) {
    ...
    if (!transaction) {
      createdTransaction = await PaymentMobileComponent.createTransaction(
        checkoutID,
        cardBrand,
        cardHolder,
        cardNumber,
    ...
    const submitTransactionCompletionHandler = checkoutTransaction => {
      if (
        checkoutTransaction.transactionType === TransactionTypes.SYNCHRONOUS
      ) {
      ...
      this.setState({
        transactionRedirectUrl: checkoutTransaction.redirectUrl,
        showModal: true
      });
      return waitForAsynchronousPaymentCallback(500, 5)
        .then(() => {
    ...
    return PaymentMobileComponent.submitTransaction(createdTransaction).then(
      submitTransactionCompletionHandler
    );
  }
  ...
  static createTransaction(
    ...
    let tempPaymentBrand = cardBrand ? cardBrand : "";
    return PaymentMobile.createTransaction(
      checkoutID,
      tempPaymentBrand,
      cardHolder,
      cardNumber,
  ...
  static createTransactionWithToken(
    checkoutID,
    tokenID,
    cardBrand,
    cardCVV
    ...
    let tempPaymentBrand = cardBrand ? cardBrand : "";
    return PaymentMobile.createTransactionWithToken(
      checkoutID,
      cardBrand,
      tokenID,
      cardCVV
  ...
  static getCheckoutId(
    ...
    console.log('body', parameterString);
    console.log('header', { headers: requestHeaders });

    return axios.post(url, parameterString, { headers: requestHeaders });
  }

  static getResourcePath() {
    return PaymentMobile.getResourcePath();
  }

  static getPaymentStatus(url, resourcePath, otherParams, requestHeaders) {
    if (!resourcePath) {
      return PaymentMobile.getResourcePath().then(resourcePath =>
        doGetPaymentStatus(url, resourcePath, otherParams, requestHeaders)
      );
    }
...
function submitTransaction(transaction) {
  return PaymentMobile.submitTransaction(transaction);
}
...
async function waitForAsynchronousPaymentCallback(
  msToWait,
  totalMinutesToWait
) {
