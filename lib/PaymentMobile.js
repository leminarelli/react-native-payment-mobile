...
export default class PaymentMobileComponent extends Component {
    ...
    async submitTransaction(transaction) {
        ...
        if (!transaction) {
            createdTransaction = await PaymentMobileComponent.createTransaction(checkoutID, paymentBrand, cardHolder, cardNumber, cardExpiryMonth, cardExpiryYear, cardCVV);
        } else {
        ...
        return PaymentMobileComponent.submitTransaction(createdTransaction).then(submitTransactionCompletionHandler);
    ...
    static createTransaction(checkoutID, paymentBrand, cardHolder, cardNumber, cardExpiryMonth, cardExpiryYear, cardCVV) {
        ...
        return PaymentMobile.createTransaction(checkoutID, tempPaymentBrand, cardHolder,cardNumber,cardExpiryMonth, cardExpiryYear, cardCVV);
    }

    static createTransactionWithToken(checkoutID, tokenID, paymentBrand, cardCVV) {
        let tempPaymentBrand = paymentBrand ? paymentBrand : '';
        return PaymentMobile.createTransactionWithToken(checkoutID, paymentBrand, tokenID, cardCVV);
    }
    ...
    static getPaymentStatus(url, resourcePath, otherParams, requestHeaders) {
        if (!resourcePath) {
            return PaymentMobile.getResourcePath().then(resourcePath => doGetPaymentStatus(url, resourcePath, otherParams, requestHeaders));
        }
